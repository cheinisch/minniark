...
<html lang="<?php echo get_language(); ?>">
  <head>
    ...
    <title>Blogpost Editor · <?php echo get_sitename(); ?></title>
    ...
    <!-- ==================== Sidebar (mobile) ==================== -->
      ...
      <main class="py-10 bg-white dark:bg-black">
        <div class="px-4 sm:px-6 lg:px-8">
          ...
          <form id="essayForm" action="backend_api/essay_save.php" method="post" class="space-y-12">
            <!-- Main Content -->
            <section class="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 sm:px-0 md:grid-cols-3">
              <div>
                <h2 class="text-sm font-semibold">Main Content</h2>
                <p class="mt-1 text-sm text-black/60 dark:text-gray-400">Displayed publicly. Choose wisely.</p>
              </div>
              <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-6 gap-x-6 gap-y-8">
                <div class="sm:col-span-4">
                  <label for="title" class="block text-sm font-medium">Title</label>
                  <div class="mt-2">
                    <div class="flex items-center bg-white pl-3 outline -outline-offset-1 outline-black/10 focus-within:outline-2 focus-within:outline-sky-600 dark:outline-white/10">
                      <input type="text" name="title" id="title"
                             class="block min-w-0 grow py-1.5 pr-3 pl-1 text-sm placeholder:text-gray-400 focus:outline-none bg-white dark:bg-black"
                             placeholder="Title" value="<?php echo $essay['title']; ?>">
                    </div>
                  </div>
                </div>

                <div class="sm:col-span-4">
                  <label for="foldername" class="block text-sm font-medium">Foldername</label>
                  <div class="mt-2">
                    <div class="flex items-center bg-white pl-3 outline -outline-offset-1 outline-black/10 focus-within:outline-2 focus-within:outline-sky-600 dark:outline-white/10">
                      <div class="shrink-0 text-sm text-black/60 select-none dark:text-gray-400">/userdata/content/essays/</div>
                      <input type="text" name="foldername" id="foldername" readonly
                             class="block min-w-0 grow py-1.5 pr-3 pl-1 text-sm placeholder:text-gray-400 focus:outline-none bg-white dark:bg-black"
                             value="<?php echo $essay['slug']; ?>">
                ...
                <div class="col-span-full">
                  ...
                  <div class="mt-2 bg-white dark:bg-black outline -outline-offset-1 outline-black/10 dark:outline-white/10">
                    <textarea name="content" id="content" rows="10"
                              class="block w-full bg-white dark:bg-black px-3 py-2 text-sm outline-none"><?php echo $essay['content']; ?></textarea>
                  </div>
                </div>
                ...
                <div class="col-span-full">
                  ...
                  <label class="block text-sm font-medium mb-1">Hero Image Preview</label>
                  <div class="w-full max-w-md overflow-hidden border border-black/10 dark:border-white/10">
                    <img id="coverPreview" src="<?php echo get_cached_image_dashboard($essay['cover'], 'M'); ?>" alt="Cover Preview" class="w-full h-48 object-cover">
                  </div>
                  <div class="mt-2 flex gap-2">
                    <button type="button" onclick="openCoverModal()"
                            class="inline-flex items-center px-3 py-2 text-sm font-semibold border border-black/20 rounded hover:bg-black/5
                                   dark:border-white/20 dark:hover:bg-white/10">
                      Select Hero Image
                    </button>
            ...
            <!-- Settings -->
            <section class="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-3">
              <div>
                <h2 class="text-sm font-semibold">Post settings</h2>
                <p class="mt-1 text-sm text-black/60 dark:text-gray/400">Visibility & date.</p>
              </div>
              <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-6 gap-x-6 gap-y-8">
                <div class="col-span-full">
                  <div class="flex items-center justify-between">
                    <span class="flex grow flex-col">
                      ...
                      <span class="text-xs text-black/60 dark:text-gray-400">Toggle visibility</span>
                    </span>
                    <button type="button" id="is_published"
                            class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent bg-gray-400 transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-sky-600 focus:ring-offset-2"
                            role="switch"
                ...
                <div class="col-span-full">
                  <label for="published_at" class="block text-sm font-medium mb-1">Publishing Date</label>
            ...
            <!-- Plugins (optional; unverändert, nur in Section gerahmt) -->
            <?php
              ...
              foreach ($pluginDirs as $pluginDir) {
                ...
                echo '<section class="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-3">';
                echo '  <div><h2 class="text-sm font-semibold">'. $pluginName .'</h2></div>';
                echo '  <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-6 gap-x-6 gap-y-8">';
                foreach ($fields as $field) {
                  ...
                  if ($type === 'toggle') {
                    ...
                    echo '      <span class="text-xs text-black/60 dark:text-gray-400">'. $hint .'</span>';
                    echo '    </span>';
                    ...
                    echo '      <label class="block text-sm font-medium mb-1">'. $label .'</label>';
            ...
            <!-- Save Bar -->
            <section class="grid max-w-7xl grid-cols-1 gap-x-8 md:grid-cols-3">
              <div>
                <h2 class="text-sm font-semibold"><?php echo languageString('general.save'); ?></h2>
              </div>
        ...
        <div tabindex="0" class="fixed inset-0 flex items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <el-dialog-panel
            ...
            <div class="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
              <button type="button" command="close" commandfor="deleteModal"
                      class="rounded-md bg-white text-gray-400 hover:text-gray-500 dark:bg-black">
            ...
            <div class="mt-6 sm:flex sm:flex-row-reverse">
              <button id="confirmDelete"
                      class="inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold border border-black/20 hover:bg-black/5 sm:ml-3 sm:w-auto
                             dark:border-white/20 dark:hover:bg-white/10">
                Delete
              </button>
              <button id="cancelDelete" type="button" command="close" commandfor="deleteModal"
                      class="mt-3 inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold border border-black/20 hover:bg-black/5 sm:mt-0 sm:w-auto
                             dark:border-white/20 dark:hover:bg-white/10">
                <?php echo languageString('general.cancel'); ?>
              </button>
            </div>
      ...
      <dialog id="coverModal" class="backdrop:bg-transparent">
        ...
        <div tabindex="0" class="fixed inset-0 flex items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <el-dialog-panel
            ...
            <h2 class="text-base font-semibold">Select Hero Image</h2>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        function slugify(text) {
          ...
          return text.toString().toLowerCase().trim()
            .replace(/[äöüßàáèéìíòóùúñ]/g, m => map[m])
            .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
        }
        async function checkAndGenerateFolder(slug) {
          const res = await fetch('backend_api/check_foldername.php?base=' + encodeURIComponent(slug));
          const data = await res.json();
          folderInput.value = data.suggested;
        }
        titleInput.addEventListener("input", () => {
          const base = slugify(titleInput.value);
          folderInput.value = base;
          if (base) checkAndGenerateFolder(base);
        });
      });
      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        toggleBtn.addEventListener("click", () => {
          const cur = toggleBtn.getAttribute("aria-checked") === "true";
          updateToggleUI(!cur);
        });
      });

      // Cover modal helpers
      function openCoverModal(){ document.getElementById('coverModal').setAttribute('open',''); }

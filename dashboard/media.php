...
<html lang="<?php echo get_language(); ?>">
	...
	<body class="bg-white dark:bg-black">
		...
		<div id="uploadModal" class="hidden fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent z-50">
			<!-- Backdrop -->
			<el-dialog-backdrop
			...
			<el-dialog-panel
				...
				<div id="progressContainer" class="mt-4 w-full rounded-full h-2.5 bg-black/5 dark:bg-white/10">
					<div id="progressBar" class="h-2.5 rounded-full bg-cyan-600 text-xs text-center text-white dark:bg-cyan-500" style="width:0%"></div>
				</div>
		...
		<div id="assignToAlbumModal" class="hidden fixed inset-0 z-50" role="dialog" aria-modal="true" aria-labelledby="assign-title">
			...
			<el-dialog-panel
				...
				<select id="albumSelect" name="album"
						class="mt-2 block w-full rounded-md bg-white/5 px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-cyan-600
				...
				<div class="mt-6 sm:flex sm:flex-row-reverse">
					<button type="submit"
							class="inline-flex w-full justify-center rounded-md bg-cyan-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-cyan-500 sm:ml-3 sm:w-auto
								dark:bg-cyan-500 dark:shadow-none dark:hover:bg-cyan-400">
					Assign
					</button>
			...
			<el-dialog-panel
				...
				<div class="space-y-4">
					...
					<input type="text" name="album-title" id="album-title" value=""
							...
							class="mt-2 block w-full rounded-md bg-white/5 px-3 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-cyan-600
		...
		<el-dialog>
			<dialog id="deleteImageModal" aria-labelledby="dialog-title" class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
				...
				<el-dialog-panel class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg sm:p-6 data-closed:sm:translate-y-0 data-closed:sm:scale-95 dark:bg-black dark:outline dark:-outline-offset-1 dark:outline-white/10">
					<div class="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
					<button type="button" command="close" commandfor="deleteImageModal" class="rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-2 focus:outline-offset-2 focus:outline-cyan-600 dark:bg-black dark:hover:text-gray-300 dark:focus:outline-white">
						<span class="sr-only">Close</span>
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" data-slot="icon" aria-hidden="true" class="size-6">
						<path d="M6 18 18 6M6 6l12 12" stroke-linecap="round" stroke-linejoin="round" />
						</svg>
